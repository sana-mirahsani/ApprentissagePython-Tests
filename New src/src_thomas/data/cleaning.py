...
def process_raw_data(input_file, output_file):
    ...
    #_warn_users_with_concurrent_sessions(logs)
    ...
    logs.to_csv(INTERIM_DATA_DIR + output_file, na_rep="", index=False)
...
# On garde tout en attendant de savoir ce qu'on en fait
#def _remove_sessions_at_the_same_time_for_actor(logs, min_session_size):
#    markers = pd.DataFrame(logs.loc[logs['verb'].isin(SESSION_MARKERS)])
#    markers = markers.sort_values(['actor', 'timestamp.$date']).reset_index#()

#    starts = markers.loc[markers['verb'] == 'Session.Start'].index
#    ends = markers.iloc[starts + 1]
#    misplaced_ends = ends.loc[ends['verb'] != 'Session.End']['session.id#']

#    ends = markers.loc[markers['verb'] == 'Session.End'].index
#    starts = markers.iloc[ends - 1]
#    misplaced_starts = starts.loc[starts['verb'] != 'Session.Start']['session.id#']

#    misplaced = np.concatenate((misplaced_starts, misplaced_ends))
#    wrong = logs.loc[logs['session.id'].isin(misplaced#)]

#    to_remove =  wrong.loc[wrong.groupby(['actor', 'session.id']).transform('size') < min_session_size #]

#    clean = logs.drop(to_remove.#index)

#    print(f"found {len(misplaced)} sessions occuring at the same time for an actor. Removed {to_remove['session.id'].nunique()} sessions with less than {min_session_size} events#")
...
def _warn_users_with_concurrent_sessions(logs):
    # selectionner les verbes Start et End
    markers = pd.DataFrame(logs.loc[logs['verb'].isin(SESSION_MARKERS)])
    # Pour chaque actor trouver les sessions imbriquées
    multiple = markers.sort_values(['timestamp.$date']).groupby(['actor'])
    embedded_sessions_per_user = multiple.apply(_detect_multiple_sessions, include_groups=False)
    # Ne conserver que les cas où il y a effectivement plusieurs sessions
    mask = embedded_sessions_per_user.apply(lambda x: len(x) > 1)

    print(f"The following actors avec concurrent sessions:\n {embedded_sessions_per_user[mask]}")
