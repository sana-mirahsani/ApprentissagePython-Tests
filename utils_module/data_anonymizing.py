...
def anonymize_data(df:pd.DataFrame, data_output_file:str, actors_output_file:str) -> None:
    """
...
def optimized_anonymize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    step = 1000
    list_anonymized_df = []
    for idx in range(0, len_df - step, step):
        print(f"Anonymizing rows {idx}-{idx+step-1}/{len_df}")
         
        try:
            list_anonymized_df.append(_slice_anonymize_dataframe(df, idx, idx + step))
            print("ok")
        except:
            print("Erorr in slicing anonmizing")

    
    print(f"Anonymizing rows {idx + step}-{len_df}/{len_df}")
    list_anonymized_df.append(_slice_anonymize_dataframe(df, idx + step, len_df))
    return pd.concat(list_anonymized_df)
...
def anonymize_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        ...
        anonymized_actors = [_hash_224(actor), '' if binome == '' else _hash_224(binome)]

        row['actor']  = anonymized_actors[0] 
        row['binome'] = anonymized_actors[1]

        for column in COLUMNS_WITH_PATH:
            row[column] = row[column].replace(actor, anonymized_actors[0])
